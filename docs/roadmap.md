...
- **Windows agent** — TCP listener that plays an incoming stream to the default device, with a pre-roll jitter buffer and backpressure ([agent.md](./agent.md)).
...
- **Authentication**, **persistence**, and **multi-agent concurrency** beyond one-stream-per-agent.
...
| **Concurrency / fan-out** | Stream one source to many agents; per-agent volume/grouping. | Today the agent handles one connection at a time and the server runs one stream per agent. |
