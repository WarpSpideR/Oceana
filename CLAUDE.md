...
- **Server** = FastEndpoints (REPR) + **vertical slices** (`Features/Agents/`, `Features/Zones/` — one folder per endpoint) with shared `Infrastructure/` (`Streaming/`, `Realtime/` SignalR, `Audio/` WAV decode). **Zones** are named groups of `(agentId, deviceId)` devices — in-memory, unique names (409 on clash); live over the `/hubs/zones` status hub. Two ways to play to a zone, sharing `ZoneTargeting.Classify` + the push-routing→stream→restore envelope (best-effort, skips offline/busy agents): **broadcast** (`POST …/broadcast`, mono mic message, one-shot) and **playback** (`POST …/play` + `DELETE`/`GET …/playback`, a stereo file, start/stop + now-playing via `ZonePlaybackService`, pushed as `ZonePlaybackChanged` on `/hubs/zones`). Both take a browser-decoded 48 kHz 16-bit PCM WAV (the agent doesn't resample); `WavReader` accepts mono/stereo 48k/16 only. Each zone has a persisted **`Volume`** (0–1, attenuation) applied server-side in the stream pump via a `Func<double>` gain read per 20 ms chunk (`PcmGain`); playback reads it **live** from the zone registry, so `PUT …/volume` changes audio already playing. FastEndpoints is **secure-by-default**, and there is no auth yet, so **every endpoint must call `AllowAnonymous()`**. CORS is likewise wide open (any origin, `SetIsOriginAllowed(_ => true)` + credentials) for the front end — tighten both when auth lands.
- **Persistence** = JSON files under a git-ignored `data/` dir (`Storage:Path`), behind `IStateStore<T>`/`JsonStateStore<T>` (atomic writes). Only **configuration** persists — zones, and agent *config* (id, last-seen devices, desired routing); **live state is not** (connection/host/port/devices/status re-derive on reconnect). Registries take the store as an **optional** ctor dep (null = no persistence) so unit tests are unaffected. Don't persist `MarkOffline`/`UpdateStatus` (live-only).
